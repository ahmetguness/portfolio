...
const EditProjectsSectionPage: React.FC = () => {
  const { projectsSectionContent, setProjectsSectionContent } = useProjectsSection();
  const navigate = useNavigate();

  const [content, setContent] = useState(projectsSectionContent);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setProjectsSectionContent(content);
    alert('Projects section updated successfully! This change is temporary and will be lost on page reload.');
    navigate('/');
  };

  return (
    <div>
      <h1 className="text-4xl font-bold mb-8 text-light-gray">Edit Projects Section</h1>
...
export default EditProjectsSectionPage;
